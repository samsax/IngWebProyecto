...
public class RespuestaService {
	...
	public void guardaRespuesta(int idRespuesta, String texto, 
		...
		if(Validaciones.isTextoVacio(idRespuesta)){
			throw new IWServiceException("La identificaci髇 de la evaluaci髇 no puede ser nula, ni una cadena de caracteres vacia");
		}
		if(Validaciones.isTextoVacio(texto)){
			throw new IWServiceException("El tiempo de evaluaci髇 no puede ser nulo, ni una cadena de caracteres vacia");
		}
	...
	public void actualizarRespuesta(int idRespuesta, String texto, 
		...
		if(Validaciones.isTextoVacio(idRespuesta)){
			throw new IWServiceException("La identificaci贸n de la evaluaci贸n no puede ser nula, ni una cadena de caracteres vacia");
		}
		if(Validaciones.isTextoVacio(texto)){
			throw new IWServiceException("El tiempo de evaluaci贸n no puede ser nulo, ni una cadena de caracteres vacia");
		}
		
		respuesta = respuestaDAO.obtenerRespuesta(idRespuesta);
		
		if(respuesta == null){
			throw new IWServiceException("La evaluaci贸n a modificar no existe en el sistema");
		}
	...
	public Respuesta obtener(int idRespuesta) throws IWDaoException, IWServiceException, MyException{
		if(idRespuesta == -1 && "".equals(idRespuesta)){
			throw new IWServiceException("La identificaci贸n de la respuesta a buscar no puede ser nula, ni una cadena de caracteres vacia");
		}
